...
int main(int argc, char** argv) {
	...
	if (iTask == 0) {
		...
		for (int iWorker = 0; iWorker < nWorkers; iWorker++) {
			...
			MPI_Send(&nRowsPerWorker, 1, MPI_INT, destination, BEGIN, MPI_COMM_WORLD);
			...
			std::cout << "Sent to process " << destination << " "
				...
				<< std::setw(3) << rowOffset * nColumns << "..."
				<< std::setw(3) << rowOffset * nColumns + nRowsThisWorker * nColumns - 1
				<< "]" << std::endl;
		...
		for (int iWorker = 0; iWorker < nWorkers; iWorker++) {
			...
			MPI_Recv(&nRowsPerWorker, 1, MPI_INT, source, msgTag, MPI_COMM_WORLD, &status);
			// - data from iWorker
			MPI_Recv(&data[rowOffset*nColumns], nRowsThisWorker* nColumns - 1, MPI_FLOAT, source, msgTag, MPI_COMM_WORLD, &status);
		...
		MPI_Recv(&data[rowOffset*nRows], 1, MPI_INT, source, msgTag, MPI_COMM_WORLD, &status);
		...
		for (int iter = 0; iter < nIterations; iter++) {
			...
			if (below != NONE) {
				...
				std::cout << " receives data[" << std::setw(3)
					...
					<< (rowOffset + nRows) * nColumns + nColumns - 1 << "] from worker "
		...
		MPI_Send(&data[rowOffset*nRows], 1, MPI_INT, msgTag, BEGIN, MPI_COMM_WORLD);
		// - row offset for this worker's data
		MPI_Send(&rowOffset, 1, MPI_INT, msgTag, BEGIN, MPI_COMM_WORLD);
		// - number of rows processed by this worker
		MPI_Send(&nRows, 1, MPI_INT, msgTag, BEGIN, MPI_COMM_WORLD);
...
void update(int startRow, int endRow, int nColumns, const float wx, const float wy,
